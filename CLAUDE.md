...
**Implementation progress (2026-04-28):** 8 / 106 tasks Done. Currently in **Phase 1: Bootstrap & Deployment Foundation** (8/17 tasks complete). Just completed: `TASK-ingress-tailscale-config` — Tailscale private-network-only ingress configured via `4-deploy/ingress/tailscale-serve.json` (18 path-prefix routes mirroring the Caddy matrix per `api-design.md`, scoped to `${TS_CERT_DOMAIN}:443` with TCP 443 HTTPS termination); `docker-compose.yml` wires `TS_SERVE_CONFIG` and mounts the file read-only into `ingress-tailscale`; `4-deploy/ingress/README.md` Tailscale section completed (operator setup with auth-key creation, routing matrix, divergences from Caddy: no URL rewrites so `/v1/health/<service>` aggregation is Caddy-only). No `tailscale funnel` exposure. Drift check clean (16 keys, unchanged — no new `.env` keys; existing `TS_AUTHKEY` / `TS_HOSTNAME` are sufficient). Previously completed: `TASK-monorepo-skeleton`, `TASK-compose-stack-skeleton`, `TASK-env-example-bootstrap`, `TASK-postgres-bootstrap`, `TASK-ollama-bootstrap`, `TASK-ingress-caddy-config`, `TASK-ci-caddyfile-validate`.
