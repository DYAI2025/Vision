...
Decision totals: 15 (2 from Spec phase + 9 from Design + 1 from Code phase: `DEC-backend-stack-python-fastapi` + 1 from Code phase tightening: `DEC-cursor-pagination-and-event-stream-conventions`, recorded 2026-04-29 to close completeness-assessment gap M-5 + 1 from Code phase: `DEC-cli-stack-python-typer`, recorded 2026-04-29 settling the second per-component tech-stack decision deferred from `DEC-backend-stack-python-fastapi` + 1 from Phase 2 first task: `DEC-shared-utility-path-deps`, recorded 2026-04-29 settling the cross-component shared-utility location explicitly deferred by `DEC-backend-stack-python-fastapi`).
...
**Completeness assessment (2026-04-27, second pass):** 0 Critical, 0 Important, 2 Minor. I-1 closed by a small `architecture.md` update assigning duplicate detection to `hermes-runtime` (with rationale). Spec-phase carry-overs M-1 and M-3 remain closed. **M-4 and M-5 closed on 2026-04-29:** M-4 via a new `## Performance characteristics` section in `architecture.md` explicitly tracing `REQ-PERF-ingest-latency` + `REQ-PERF-routing-throughput` to per-component mechanisms; M-5 via `DEC-cursor-pagination-and-event-stream-conventions` formalizing the API conventions previously documented only in `api-design.md` prose, plus a new `REQ-USA-paginated-lists` (Approved per Vincent concurrence) making the conventions verifiable. **Result: 0 Critical, 0 Important, 0 Minor — completeness assessment fully closed.** All 10 approved requirements covered across the 3 design docs; 24/25 Draft+Approved-Should-have requirements covered (`REQ-F-duplicate-detection` design-anchored to `hermes-runtime`; new `REQ-USA-paginated-lists` design-anchored to all four backend HTTP services); all 10 constraints verified with no violations; 8 unverified assumptions including the new `ASM-no-scalability-target` (Business, Medium risk; explicitly deprioritizes horizontal scaling for MVP and closes the `REQ-SCA` 0-artifact gap as a deliberate scope choice); 13 decisions recorded. **Component decomposition (2026-04-28):** 6 components identified and per-component directories created under `3-code/`: `whatsorga-ingest`, `hermes-runtime`, `backlog-core`, `gbrain-bridge`, `kanban-sync`, `cli`. Each component carries a `CLAUDE.component.md`. Per-component technology choices deferred to Code-phase decisions when the first implementation task per component is picked up.

**Implementation progress (2026-05-02):** 19 / 107 tasks Done. **Phase 1: Bootstrap & Deployment Foundation is complete (17/17). Phase 2: Consent Foundation + Audit Backbone is in progress (2/16).** Just completed: `TASK-bearer-auth-middleware` (Phase 2 #2) — second cross-cutting helper under `3-code/_common/`, scoped to **inbound authentication only** per the Phase-2/Phase-3 split (purpose-limitation enforcement is `TASK-purpose-limitation-middleware`, next). New package `_common/bearer_auth/` (37/37 tests green) ships: `CallingIdentity` frozen dataclass; `BearerAuthVerifier` with `hmac.compare_digest` constant-time match; `AcceptedTokens` env-driven config loader using the `<NAME>_TOKEN` convention; `MissingAuthError` / `InvalidAuthError` mapping to api-design.md `auth_required` / `auth_invalid` (HTTP 401); and the `require_bearer_auth` FastAPI dependency that attaches `request.state.calling_identity` for the next-task purpose middleware to read. Tests cover `DEC-service-auth-bearer-tokens` § "Required checks" 1+2 (presence/recognition + malformed-header rejection); a regression guard verifies `/v1/health` stays unauthenticated per api-design.md. Defense-in-depth: missing verifier state returns 401 (fail-closed); duplicate-token across distinct identities raises `ValueError` at startup. All 5 backend `pyproject.toml` files declare `bearer-auth` path-dep `editable = true`; per-component `uv.lock` files regenerated (each adds bearer-auth at v0.0.1; starlette pinned to 0.41.3 within FastAPI's compatible range). New CI job `_common-bearer-auth-test` (13 jobs total now); each consuming component's `cache-dependency-glob` adds the new lockfile. Backend Dockerfiles unchanged — the repo-root build context + `_common/` copy already established by `TASK-canonical-json-helper` accommodates additional `_common/` packages. 67/67 backend skeleton tests still green. No new decisions recorded (15 active total). Pre-existing observation correction (closed by [`docs/plans/2026-05-02-backlog-core-cast-quotes-followup.md`](docs/plans/2026-05-02-backlog-core-cast-quotes-followup.md)): an earlier draft of this paragraph claimed repo-root ruff reports 2 `TC006` errors in `backlog-core/app/db.py`. On verification this is not reproducible — ruff 0.7.4 (our pinned version) does not ship a `TC006` rule, and the per-component CI invocation is clean. The follow-up plan applied the forward-compatible `cast("_PoolLike", pool)` quoted-string form anyway as future-proofing. Earlier in Phase 2: `TASK-canonical-json-helper` shipped the `_common/<package>/` carve-out via `DEC-shared-utility-path-deps` (15th active decision); first helper `_common/canonical_json/` (20/20 tests green); all 5 backend Dockerfiles restructured to repo-root build context (the structure that lets `_common/bearer_auth/` slot in without Dockerfile changes today). Next Phase 2 task: `TASK-postgres-events-schema` (P1; depends on `TASK-backlog-core-skeleton` ✓; defines the `events` table per `data-model.md` and unblocks the entire audit-backbone chain). Previously completed in Phase 1: `TASK-monorepo-skeleton`, `TASK-compose-stack-skeleton`, `TASK-env-example-bootstrap`, `TASK-postgres-bootstrap`, `TASK-ollama-bootstrap`, `TASK-ingress-caddy-config`, `TASK-ingress-tailscale-config`, `TASK-ci-caddyfile-validate`, `TASK-whatsorga-skeleton`, `TASK-hermes-skeleton`, `TASK-backlog-core-skeleton`, `TASK-gbrain-bridge-skeleton`, `TASK-kanban-sync-skeleton`, `TASK-cli-skeleton`, `TASK-install-vps-script`, `TASK-smoke-test-skeleton`, `TASK-phase-1-manual-testing`. Earlier this session: Codex PR #2 was reverted (`fc1eb2e`) for architectural non-compliance with `DEC-postgres-as-event-store` + 3 other DECs.

**Implementation plan created (2026-04-28):** 7 phases, **107 tasks** in `3-code/tasks.md` covering all 5 approved goals:
- **Phase 1** (17 tasks) — Bootstrap & deployment foundation: empty deployable system, `vision health` aggregator, ingress profile, CI Caddyfile validation.
...
- **Phase 7** (20 tasks) — Multi-channel & operability: WhatsApp/voice/repo adapters, daily reconciliation, vault audit sweep, backup/restore, secret rotation, cross-provider VPS verification, tailscale-mode health aggregation, performance load tests, remote-inference profile.
