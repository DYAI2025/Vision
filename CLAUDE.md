...
Decision totals: 14 (2 from Spec phase + 9 from Design + 1 from Code phase: `DEC-backend-stack-python-fastapi` + 1 from Code phase tightening: `DEC-cursor-pagination-and-event-stream-conventions`, recorded 2026-04-29 to close completeness-assessment gap M-5 + 1 from Code phase: `DEC-cli-stack-python-typer`, recorded 2026-04-29 settling the second per-component tech-stack decision deferred from `DEC-backend-stack-python-fastapi`).
...
**Implementation progress (2026-04-29):** 17 / 107 tasks Done (Phase 7 task count rose 19 → 20 with the addition of `TASK-tailscale-health-aggregation` from the 2026-04-29 review). **Phase 1: Bootstrap & Deployment Foundation is complete (17/17).** Just completed: `TASK-phase-1-manual-testing` — new `4-deploy/runbooks/install.md` (~280 lines, the canonical install runbook with prerequisites table + 5-step procedure + exit-code contracts for `install_vps.sh` and `smoke_test.sh` + 4 manual verification scenarios + tailscale-mode operator path + troubleshooting + rollback + GDPR-deferral gate per `DEC-gdpr-legal-review-deferred`). Root `README.md` refreshed: status, install quickstart, tech-stack DEC references. Per-component READMEs spot-checked: all 6 (5 backend + cli) have consistent shape. **Phase 1 capabilities now verifiably delivered:** fresh-VPS install completes from clean clone via `scripts/install_vps.sh`; all 6 services come up via `docker-compose up`; `vision health` aggregates status across the stack; Caddy or Tailscale ingress switchable via `.env`; default deployment makes 0 remote inference calls. Phase-1 milestone summary — 14 decisions recorded (2 spec carry-over + 9 design + 3 code-phase: `DEC-backend-stack-python-fastapi`, `DEC-cursor-pagination-and-event-stream-conventions`, `DEC-cli-stack-python-typer`); 11 CI jobs (4 infra-validation + 6 component-test + 1 scripts-lint); 13 active artifacts in `docs/plans/` + `docs/reviews/`. Next phase (`Phase 2: Consent Foundation + Audit Backbone`, 16 tasks) implements source registration, hash-chained audit log, consent-history reads. Previously completed in Phase 1: `TASK-monorepo-skeleton`, `TASK-compose-stack-skeleton`, `TASK-env-example-bootstrap`, `TASK-postgres-bootstrap`, `TASK-ollama-bootstrap`, `TASK-ingress-caddy-config`, `TASK-ingress-tailscale-config`, `TASK-ci-caddyfile-validate`, `TASK-whatsorga-skeleton`, `TASK-hermes-skeleton`, `TASK-backlog-core-skeleton`, `TASK-gbrain-bridge-skeleton`, `TASK-kanban-sync-skeleton`, `TASK-cli-skeleton`, `TASK-install-vps-script`, `TASK-smoke-test-skeleton`. Earlier this session: Codex PR #2 was reverted (`fc1eb2e`) for architectural non-compliance with `DEC-postgres-as-event-store` + 3 other DECs.
