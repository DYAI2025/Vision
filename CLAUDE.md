...
**Implementation progress (2026-04-28):** 12 / 106 tasks Done. Currently in **Phase 1: Bootstrap & Deployment Foundation** (12/17 tasks complete). Just completed: `TASK-gbrain-bridge-skeleton` — fourth per-component skeleton, applies [`DEC-backend-stack-python-fastapi`](decisions/DEC-backend-stack-python-fastapi.md). `3-code/gbrain-bridge/` mirrors the established template plus `app/vault.py` — `vault_path()` reads `VAULT_PATH` from env (default `/vault`), `is_readable(path)` returns True iff the path exists, is a directory, and is iterable (never raises, fold permission errors into False). `/v1/health` does a live vault-readability check, returning **HTTP 503 with `status: "degraded"`, `checks.vault: "down"`** when the mount is missing/unreadable — mirrors the 503-on-degraded pattern hardened on backlog-core. Tests (14/14 green) use pytest's `tmp_path` so no real `/vault` mount is required. ruff clean, mypy strict clean. New CI job `gbrain-bridge-test` mirrors prior templates. Note: a 4-commit cleanup pass on `TASK-backlog-core-skeleton` (commits `f359eea..04c7b0d`) closed all six MEDIUM/LOW findings from its 2026-04-28 code review (503 on degraded, tightened `_PoolLike` Protocol, env-tunable pool size with new `BACKLOG_CORE_DB_POOL_MIN/MAX` keys → 18 drift keys total, simplified `get_pool` signature, consolidated test fixtures); plan recorded at `docs/plans/2026-04-28-backlog-core-review-fixes.md`. Previously completed: `TASK-monorepo-skeleton`, `TASK-compose-stack-skeleton`, `TASK-env-example-bootstrap`, `TASK-postgres-bootstrap`, `TASK-ollama-bootstrap`, `TASK-ingress-caddy-config`, `TASK-ingress-tailscale-config`, `TASK-ci-caddyfile-validate`, `TASK-whatsorga-skeleton`, `TASK-hermes-skeleton`, `TASK-backlog-core-skeleton`.
