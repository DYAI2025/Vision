...
**Implementation progress (2026-04-28):** 4 / 105 tasks Done. Currently in **Phase 1: Bootstrap & Deployment Foundation** (4/16 tasks complete). Just completed: `TASK-postgres-bootstrap` — `4-deploy/postgres/` operator-facing bootstrap (`README.md` documenting service config + init semantics + `psql.sh` helper; `init/` mount point with self-documenting README, currently empty by design since schema lands in Phase 2 via migrations); `scripts/psql.sh` (executable wrapper around `docker compose exec postgres psql ...`); `docker-compose.yml` updated to mount `4-deploy/postgres/init/` read-only into the Postgres container at `/docker-entrypoint-initdb.d/`. Previously completed: `TASK-monorepo-skeleton`, `TASK-compose-stack-skeleton`, `TASK-env-example-bootstrap`.
