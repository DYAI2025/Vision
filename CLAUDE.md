...
**Implementation progress (2026-04-28):** 11 / 106 tasks Done. Currently in **Phase 1: Bootstrap & Deployment Foundation** (11/17 tasks complete). Just completed: `TASK-backlog-core-skeleton` — third per-component skeleton; first to wire a real downstream dependency. Applies [`DEC-backend-stack-python-fastapi`](decisions/DEC-backend-stack-python-fastapi.md) + `DEC-postgres-as-event-store`. `3-code/backlog-core/` template adds asyncpg `>=0.30,<0.31` (`uv.lock` 36 packages) plus `app/db.py` — `@asynccontextmanager` lifespan that creates the pool from `DATABASE_URL` (fail-fast if unset per `REQ-MNT-env-driven-config`), `_PoolLike` Protocol for test fakes, `get_pool` dependency, and a non-raising `ping()` running `SELECT 1`. `/v1/health` now reports `{"checks": {"postgres": "ok|down"}}` and degrades to `status: "degraded"` when Postgres is unreachable. `app/main.py` uses the modern `Annotated[_PoolLike, Depends(...)]` pattern (vs. `Depends()` in defaults). Tests (11/11): `tests/conftest.py` ships an in-process `FakePool` so unit tests need no Postgres process — 6 health-endpoint cases (200, shape, ok, degraded, no-auth, 404) + 5 db-module cases. ruff clean, mypy strict clean (asyncpg missing-stubs handled via `[[tool.mypy.overrides]]`). New CI job `backlog-core-test`. Schema, idempotency store, hash-chain audit, RTBF cascade, retention sweep, etc. defer to Phase 2 / Phase 3 / Phase 4 / Phase 6 / Phase 7. Previously completed: `TASK-monorepo-skeleton`, `TASK-compose-stack-skeleton`, `TASK-env-example-bootstrap`, `TASK-postgres-bootstrap`, `TASK-ollama-bootstrap`, `TASK-ingress-caddy-config`, `TASK-ingress-tailscale-config`, `TASK-ci-caddyfile-validate`, `TASK-whatsorga-skeleton`, `TASK-hermes-skeleton`.
