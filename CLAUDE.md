...
Decision totals: 13 (2 from Spec phase + 9 from Design + 1 from Code phase: `DEC-backend-stack-python-fastapi` + 1 from Code phase tightening: `DEC-cursor-pagination-and-event-stream-conventions`, recorded 2026-04-29 to close completeness-assessment gap M-5).
...
**Implementation progress (2026-04-29):** 13 / 106 tasks Done. Currently in **Phase 1: Bootstrap & Deployment Foundation** (13/17 tasks complete). Just completed: `TASK-kanban-sync-skeleton` — **fifth and final** per-component skeleton, applies [`DEC-backend-stack-python-fastapi`](decisions/DEC-backend-stack-python-fastapi.md). `3-code/kanban-sync/` mirrors the established template plus `app/kanban.py` — filesystem layer scoped to the **Kanban subtree** specifically (`<VAULT_PATH>/Kanban/`, default `/vault/Kanban`), with `vault_path()` for read-only project-page link resolution and `is_writable(path)` (`os.access` R+W check, never raises) for the writable carve-out. `/v1/health` returns **HTTP 503 with `status: "degraded"`, `checks.kanban_subtree: "down"`** when missing/unwritable — mirrors backlog-core/gbrain-bridge. Tests (19/19 green) use pytest's `tmp_path`. ruff clean, mypy strict clean. New CI job `kanban-sync-test` is the fifth and last backend per-component template instance. Skeleton intentionally does **not** auto-create the subtree — misconfiguration must be visible. **All 5 backend skeletons now landed** (whatsorga-ingest, hermes-runtime, backlog-core, gbrain-bridge, kanban-sync); only the `cli` component's TBD-tech-stack skeleton (`TASK-cli-skeleton`) remains in the per-component skeleton track. Recent context: a Codex PR #2 was merged on origin during the 2026-04-29 session and reverted (`fc1eb2e`) for architectural non-compliance with `DEC-postgres-as-event-store`, `DEC-confidence-gate-as-middleware`, `DEC-service-auth-bearer-tokens`, and the just-recorded `DEC-cursor-pagination-and-event-stream-conventions`. Codex history preserved; non-destructive revert. Previously completed: `TASK-monorepo-skeleton`, `TASK-compose-stack-skeleton`, `TASK-env-example-bootstrap`, `TASK-postgres-bootstrap`, `TASK-ollama-bootstrap`, `TASK-ingress-caddy-config`, `TASK-ingress-tailscale-config`, `TASK-ci-caddyfile-validate`, `TASK-whatsorga-skeleton`, `TASK-hermes-skeleton`, `TASK-backlog-core-skeleton`, `TASK-gbrain-bridge-skeleton`.
