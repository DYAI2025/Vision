...
**Implementation progress (2026-04-29):** 14 / 106 tasks Done. Currently in **Phase 1: Bootstrap & Deployment Foundation** (14/17 tasks complete). Just completed: `TASK-cli-skeleton` — **sixth and final per-component skeleton**, established the second per-component tech-stack convention via new [`DEC-cli-stack-python-typer`](decisions/DEC-cli-stack-python-typer.md) (Python 3.12 + Typer + httpx + Pydantic + python-dotenv + Rich; primary distribution via `uv tool install`, secondary via profile-gated `cli` Compose service for in-stack invocation). `3-code/cli/` ships `app/main.py` (Typer app + `vision health` + global `--version`), `app/config.py` (env / `.env`-driven base URL + `OPERATOR_TOKEN` loader with walk-upward `.env` discovery), `app/health.py` (parallel-fan-out aggregator using `asyncio.gather` over the 5 backend services' Caddy `/v1/health/<service>` paths; `ServiceHealth` dataclass; classifier for 200/503/4xx/non-JSON/connection-failure variants; `overall_status` reducer; `to_json` renderer). Tests (19/19 green): `tests/test_config.py` 7 cases (env / `.env` / arg precedence + walk-up + empty-token); `tests/test_health.py` 12 cases via `httpx.MockTransport` (no live network). ruff clean, mypy strict clean. New CI job `cli-test` mirrors the 5 backend templates; compose-validate also validates the new `cli` profile. Caddy-mode-only at the skeleton level; tailscale-mode rewrite limitation documented for future hardening. **Phase 1 milestone — all 6 component skeletons complete** (whatsorga-ingest, hermes-runtime, backlog-core, gbrain-bridge, kanban-sync, cli). Remaining Phase 1 tasks are operational glue: TASK-install-vps-script, TASK-smoke-test-skeleton, TASK-phase-1-manual-testing. Recent context: a Codex PR #2 was merged on origin during the 2026-04-29 session and reverted (`fc1eb2e`) for architectural non-compliance with `DEC-postgres-as-event-store`, `DEC-confidence-gate-as-middleware`, `DEC-service-auth-bearer-tokens`, and `DEC-cursor-pagination-and-event-stream-conventions`. Codex history preserved; non-destructive revert. Previously completed: `TASK-monorepo-skeleton`, `TASK-compose-stack-skeleton`, `TASK-env-example-bootstrap`, `TASK-postgres-bootstrap`, `TASK-ollama-bootstrap`, `TASK-ingress-caddy-config`, `TASK-ingress-tailscale-config`, `TASK-ci-caddyfile-validate`, `TASK-whatsorga-skeleton`, `TASK-hermes-skeleton`, `TASK-backlog-core-skeleton`, `TASK-gbrain-bridge-skeleton`, `TASK-kanban-sync-skeleton`.

**Implementation plan created (2026-04-28):** 7 phases, **106 tasks** in `3-code/tasks.md` covering all 5 approved goals:
