...
**Implementation progress (2026-04-28):** 10 / 106 tasks Done. Currently in **Phase 1: Bootstrap & Deployment Foundation** (10/17 tasks complete). Just completed: `TASK-hermes-skeleton` — second per-component skeleton, applies [`DEC-backend-stack-python-fastapi`](decisions/DEC-backend-stack-python-fastapi.md). `3-code/hermes-runtime/` mirrors `whatsorga-ingest`'s template (multi-stage `Dockerfile` with `python:3.12-slim` runtime, non-root UID 1000, port 8000; `pyproject.toml` + `uv.lock` 35 packages frozen; `app/main.py` FastAPI app exposing `GET /v1/health` per `api-design.md`; `tests/test_health.py` 5 cases) plus `app/ollama_client.py` — an async `OllamaClient` wrapping `POST /api/generate` and `POST /api/embeddings` per `hermes-runtime/CLAUDE.component.md` interfaces; reads `OLLAMA_URL` / `OLLAMA_MODEL` from env (defaults: `http://ollama:11434`, `gemma3:4b`); custom `OllamaError` for malformed payloads. Tests cover 5 health-endpoint cases + 9 OllamaClient cases via `httpx.MockTransport` (no Ollama process required) — 14/14 green; ruff clean; mypy strict clean. New CI job `hermes-runtime-test` mirrors `whatsorga-ingest-test`. Audit-log emission for remote-inference profiles deferred to `TASK-model-router` / `TASK-remote-inference-profile` per `REQ-SEC-remote-inference-audit`. Previously completed: `TASK-monorepo-skeleton`, `TASK-compose-stack-skeleton`, `TASK-env-example-bootstrap`, `TASK-postgres-bootstrap`, `TASK-ollama-bootstrap`, `TASK-ingress-caddy-config`, `TASK-ingress-tailscale-config`, `TASK-ci-caddyfile-validate`, `TASK-whatsorga-skeleton`.
