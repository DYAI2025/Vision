...
**Implementation progress (2026-04-28):** 9 / 106 tasks Done. Currently in **Phase 1: Bootstrap & Deployment Foundation** (9/17 tasks complete). Just completed: `TASK-whatsorga-skeleton` — first per-component skeleton; established the uniform backend stack via new [`DEC-backend-stack-python-fastapi`](decisions/DEC-backend-stack-python-fastapi.md) (Python 3.12 + FastAPI + uvicorn + Pydantic + asyncpg + uv + pytest + httpx + ruff + mypy strict, applied to all 5 backend components — `whatsorga-ingest`, `hermes-runtime`, `backlog-core`, `gbrain-bridge`, `kanban-sync`; CLI deferred). `3-code/whatsorga-ingest/` now contains: `pyproject.toml` (deps + lint/type/test config), `uv.lock` (35 packages, frozen), `.python-version=3.12`, `app/main.py` (FastAPI app exposing `GET /v1/health` per `api-design.md`), `app/__init__.py` (version via `importlib.metadata`), `tests/test_health.py` (5 cases: 200, payload shape, `status=ok`, no-auth, 404 on unknown path — all green: ruff clean, mypy strict clean, pytest 5/5), multi-stage `Dockerfile` (`python:3.12-slim`, non-root UID 1000, port 8000), `.dockerignore`, `.gitignore`, `README.md`. New CI job `whatsorga-ingest-test` runs `setup-uv` (cached on `uv.lock`) → `uv sync --frozen` → ruff → mypy → pytest. All five backend `CLAUDE.component.md` files and `3-code/CLAUDE.code.md` updated to reflect the uniform stack. Previously completed: `TASK-monorepo-skeleton`, `TASK-compose-stack-skeleton`, `TASK-env-example-bootstrap`, `TASK-postgres-bootstrap`, `TASK-ollama-bootstrap`, `TASK-ingress-caddy-config`, `TASK-ingress-tailscale-config`, `TASK-ci-caddyfile-validate`.
