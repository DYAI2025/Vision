...
**Implementation progress (2026-04-29):** 15 / 106 tasks Done. Currently in **Phase 1: Bootstrap & Deployment Foundation** (15/17 tasks complete). Just completed: `TASK-install-vps-script` — `scripts/install_vps.sh` brings up the full Compose stack from a clean clone, idempotent, with named exit codes (1=prereq missing, 2=drift, 3=health timeout). 12-step flow: prereq verification (`docker` + `docker compose` v2) → repo-root sanity → `.env` existence + drift check + 8 required-key value check → profile-specific `TS_AUTHKEY` check → `docker compose pull` + `build` + `up -d` → 5-min healthcheck wait across the 7 expected containers (postgres, ollama, 5 backend services) → `scripts/ollama-pull.sh` → next-steps printout pointing at `vision health`. New CI job `scripts-lint` runs `bash -n` + `shellcheck` on every `scripts/*.sh` (now 6 scripts; shellcheck preinstalled on ubuntu-latest). Per `REQ-PORT-vps-deploy`: no host-specific patches, default deployment makes zero remote inference calls per `CON-local-first-inference`. Local: `bash -n` clean on all 6 scripts; full integration verification on a real VPS deferred to `TASK-phase-1-manual-testing` (#17). **Only 2 Phase 1 tasks left:** `TASK-smoke-test-skeleton` (#16) + `TASK-phase-1-manual-testing` (#17). Previously completed: `TASK-monorepo-skeleton`, `TASK-compose-stack-skeleton`, `TASK-env-example-bootstrap`, `TASK-postgres-bootstrap`, `TASK-ollama-bootstrap`, `TASK-ingress-caddy-config`, `TASK-ingress-tailscale-config`, `TASK-ci-caddyfile-validate`, `TASK-whatsorga-skeleton`, `TASK-hermes-skeleton`, `TASK-backlog-core-skeleton`, `TASK-gbrain-bridge-skeleton`, `TASK-kanban-sync-skeleton`, `TASK-cli-skeleton`. Earlier this session: Codex PR #2 was reverted (`fc1eb2e`) for architectural non-compliance with `DEC-postgres-as-event-store` + 3 other DECs.
