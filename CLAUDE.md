...
**Implementation progress (2026-04-28):** 6 / 105 tasks Done. Currently in **Phase 1: Bootstrap & Deployment Foundation** (6/16 tasks complete). Just completed: `TASK-ingress-caddy-config` — Caddy reverse-proxy configured via `4-deploy/ingress/Caddyfile` with path-based routing per `api-design.md` (per-service `/v1/health/<service>` aggregation rewrite + backlog-core / gbrain-bridge / kanban-sync / hermes-runtime path namespaces; root and 404 handlers); `4-deploy/ingress/README.md` operator reference (routing matrix, localhost-vs-public hostname auto-TLS guidance); `.env.example` extended with `CADDY_HOSTNAME` (default `localhost`) and `CADDY_ACME_EMAIL`; compose mounts Caddyfile read-only and injects the new env vars into `ingress-caddy`. Drift check now at 16 keys. Previously completed: `TASK-monorepo-skeleton`, `TASK-compose-stack-skeleton`, `TASK-env-example-bootstrap`, `TASK-postgres-bootstrap`, `TASK-ollama-bootstrap`.
