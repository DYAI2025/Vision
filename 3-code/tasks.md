...
| TASK-idempotency-middleware | Shared `Idempotency-Key` middleware + idempotency store | P0 | Done | - | TASK-bearer-auth-middleware, TASK-postgres-events-schema | 2026-05-07 | Implemented `IdempotencyMiddleware` in `backlog-core` using `idempotency_keys` table. Verified with integration tests. |
...
| TASK-hash-chain-verify | Chain-verification routine + secondary integrity check | P1 | Done | [REQ-SEC-audit-log](../1-spec/requirements/REQ-SEC-audit-log.md) | TASK-event-emit-primitive | 2026-05-07 | Implemented in `app/audit.py` with batch verification for scalability. |
...
| TASK-audit-query-endpoint | `GET /v1/audit/query` with filters + pagination | P1 | Done | [REQ-SEC-audit-log](../1-spec/requirements/REQ-SEC-audit-log.md), [REQ-USA-paginated-lists](../1-spec/requirements/REQ-USA-paginated-lists.md) | TASK-event-emit-primitive | 2026-05-07 | Implemented in `app/main.py` with cursor-based pagination. |
| TASK-audit-verify-chain-endpoint | `POST /v1/audit/verify-chain` | P1 | Done | [REQ-SEC-audit-log](../1-spec/requirements/REQ-SEC-audit-log.md) | TASK-hash-chain-verify | 2026-05-07 | Implemented in `app/main.py`. |
| TASK-input-event-endpoint | `POST /v1/inputs` writing `input.received` events | P1 | Done | [REQ-F-input-event-normalization](../1-spec/requirements/REQ-F-input-event-normalization.md) | TASK-event-emit-primitive, TASK-idempotency-middleware | 2026-05-07 | Implemented in `app/main.py` as a generic intake for the Idea Workbench. |
| TASK-proposal-pipeline-endpoint | `POST /v1/proposals` with `gate_inputs` validation | P1 | Done | [REQ-F-proposal-pipeline](../1-spec/requirements/REQ-F-proposal-pipeline.md), [REQ-F-confidence-gate](../1-spec/requirements/REQ-F-confidence-gate.md) | TASK-event-emit-primitive, TASK-idempotency-middleware, TASK-purpose-limitation-middleware | 2026-05-07 | Implemented in `app/main.py` with `ProposalCreateRequest` validation. |
...
| TASK-kanban-card-crud | `POST/GET/PATCH/DELETE /v1/cards` | P1 | Done | [REQ-USA-kanban-obsidian-fidelity](../1-spec/requirements/REQ-USA-kanban-obsidian-fidelity.md) | TASK-kanban-sync-skeleton, TASK-idempotency-middleware | 2026-05-07 | Implemented with filesystem locking and YAML hardening. |
...
| TASK-kanban-sync-trigger | `POST /v1/sync` periodic + on-demand | P1 | Done | [REQ-USA-kanban-obsidian-fidelity](../1-spec/requirements/REQ-USA-kanban-obsidian-fidelity.md) | TASK-kanban-card-crud | 2026-05-07 | Sync now manages the central `09_Inbox/Review.md` dashboard and detects manual checkbox dispositions. |
