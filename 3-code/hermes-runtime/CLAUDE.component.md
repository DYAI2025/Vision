...
| [DEC-cursor-pagination-and-event-stream-conventions](../../decisions/DEC-cursor-pagination-and-event-stream-conventions.md) | Cursor pagination + long-poll/SSE event-stream | Consumer side: `TASK-hermes-events-consumer` reconnects with `?last_event_id=` and respects 30 s heartbeat / 5 min cap; any list endpoint hermes-runtime calls (e.g., `GET /v1/sources/:id/history`) paginates via `?after=<cursor>` until `next_cursor: null` |
