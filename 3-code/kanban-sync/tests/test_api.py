...
def test_sync_detects_move(client: TestClient) -> None:
    # Mock backlog-core endpoints
    respx.get("http://backlog-core:8000/v1/proposals?status=pending").mock(
        return_value=Response(200, json=[])
    )
    respx.post("http://backlog-core:8000/v1/inputs").mock(
        return_value=Response(201, json={"event_id": "test-ev"})
    )
